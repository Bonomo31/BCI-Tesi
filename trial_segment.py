...
def create_signal_target_from_raw_mne(
    ...
    """
    data = raw.get_data()
    print("Dati grezzi CST_raw_mne:", data.shape)
    
    # Verifica i canali di stimolazione
    stim_channels = mne.pick_types(raw.info, stim=True)
    print("Canali di stimolazione trovati:", [raw.info['ch_names'][i] for i in stim_channels])

    # Se nessun canale di stimolazione è trovato, forza 'STI 014' come stim
    if len(stim_channels) == 0:
        if 'STI 014' in raw.info['ch_names']:
            print("Forzo il canale 'STI 014' come stim.")
            raw.set_channel_types({'STI 014': 'stim'})
        else:
            raise ValueError("Il canale 'STI 014' non è presente nei dati.")
    
    # Estrai gli eventi dal canale 'STI 014'
    events = mne.find_events(raw, stim_channel='STI 014',min_duration=0.002, verbose=True,uint_cast=True)
    print("Eventi estratti:", events)
    # Filtra gli eventi con una durata minima

    # Controlla se gli eventi sono vuoti
    if len(events) == 0:
        raise ValueError("Nessun evento trovato nel canale 'STI 014'. Controlla i dati.")


    # Controlla i dati del canale 'STI 014'
    stim_data = raw.get_data(picks='STI 014')
    print("Dati del canale 'STI 014':", stim_data)
    
    # Se il canale è vuoto, solleva un errore
    if np.all(stim_data == 0):
        raise ValueError("Il canale 'STI 014' è vuoto. Non ci sono eventi registrati.")

    print("Intervallo epoca:", epoch_ival_ms)
    print("Codici di inizio:", name_to_start_codes)
...
def create_signal_target(data, events, fs, name_to_start_codes, epoch_ival_ms,
    ...
    """
    print("Dati in create_signal_target:", data.shape)
...
def _create_signal_target_from_start_and_ival(
    ...
    print("DEBUG--CREATE_SIGNAL_TARGET_FROM_START_AND_IVAL")
        # Debugging: Forma dei dati grezzi e degli eventi
    print("Dati in _create_signal_target_from_start_and_ival:", data.shape)
    print("Eventi:", len(events))
    ...
    print("cnt_y:", cnt_y)
    print("i_start_stops:", i_start_stops)
    ...
    #Aggiungi un print per verificare se X è una lista
    print("Tipo di signal_target.X:", type(signal_target.X))
    ...
    print("Signal Target creato. Shape di X:", signal_target.X.shape)
    print("Shape di y:", signal_target.y.shape)
...
def _create_cnt_y_and_trial_bounds_from_start_and_ival(
    ...
    print("mrk_code_to_name_and_y:", mrk_code_to_name_and_y)
    # DEBUG: Stampiamo i primi valori di events
    print(f"events len: {len(events)}")
    print("Prime righe di events:", events[:10])
    # Estrai i valori dalla seconda e terza colonna di ogni array in events
    event_types = [event[0] for event in events]  # prima colonna (indice 1)
    event_codes = [event[2] for event in events]  # terza colonna (indice 2)

    print(f"Tipi di dati in events[:, 0]: {set(event_types)}")
    print(f"Tipi di dati in events[:, 2]: {set(event_codes)}")
    events = np.asarray(events)


    for i_sample, mrk_code in zip(events[:, 0], events[:, 2]):

        start_sample = int(i_sample) + start_offset
        stop_sample = int(i_sample) + stop_offset
         # Log the start and stop for each trial
        print(f"Processing trial: mrk_code={mrk_code}, start_sample={start_sample}, stop_sample={stop_sample}")
        ...
        if mrk_code in mrk_code_to_name_and_y:
            ...
            #Da controllare restituisce un array di zeri 
            cnt_y[start_sample:stop_sample, this_y] = 1
            class_to_n_trials[name] += 1
    print("cnt_y:", cnt_y[:10])  # Print first few rows for inspection
...
def _create_signal_target_from_start_and_stop(
    ...
    #Debug 
    print("DEBUG--CREATE_SIGNAL_TARGET_FROM_START_AND_STOP")
    print("Controllo uguaglianza tra start e stop codes:")
    print(np.array_equal(list(name_to_start_codes.keys()), list(name_to_stop_codes.keys())))
...
def _create_signal_target_from_cnt_y_start_stops(
    ...
    print("new_i_start_stops:", new_i_start_stops)
    ...
    print("New y after processing:", y)
    ...
    # Create signal target and return it
    signal_target = SignalAndTarget(X, y)

    # Verifica se X e y sono array numpy
    print("Tipo di X:", type(signal_target.X))
    print("Tipo di y:", type(signal_target.y))

    signal_target.X = np.array(signal_target.X, dtype=np.float32)
    signal_target.y = np.array(signal_target.y, dtype=np.int64)

    print("Signal target X shape:", signal_target.X.shape)
    print("Signal target y shape:", signal_target.y.shape)
